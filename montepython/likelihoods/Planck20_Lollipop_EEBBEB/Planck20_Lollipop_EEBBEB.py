...
class Planck20_Lollipop_EB(Likelihood):
    def __init__(self, path, data, command_line):
        Likelihood.__init__(self, path, data, command_line)
         
        #Create Cobaya likelihood        
        self.lik = planck_2020_lollipop.lowlEB({"packages_path": packages_path, "Nsim": self.Nsim, "lmin":self.lmin, "lmax":self.lmax})

        self.need_cosmo_arguments(
            data, {'lensing': 'yes', 'output': 'pCl lCl', 'non_linear': 'halofit', 'l_max_scalars': self.lik.bins.lmax})

        print( "Init Lollipop (lowlEB) done !")
